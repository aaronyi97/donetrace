...
export const MESSAGES = {
  en: {
    ...
    "init.created.body":
      ...
      "Want to see this work on YOUR own recent work? Run: node bin/ai-collab.js bootstrap --yes (a local, read-only \"AI collaboration baseline\" report — nothing is written or sent anywhere).\n" +
...
New here? These three commands cover most of what you need (plain-language first):
  ...
  bootstrap  Look at your OWN recent work and print a read-only "where you stand" report. Run this right after init.
...
Usage (after publish; before publish prefix each with "node bin/ai-collab.js "):
  ...
  bootstrap: the recommended first run after init. It reads your OWN recent work --
  ...
  three cards: VERIFY (which "done"s cannot be trusted yet -- pending, risk-not-
  ...
  missing handoff, files you keep re-touching), and HARVEST (confirmed lessons you
...
`
  ...
  zh: {
    ...
    "init.created.body":
      ...
      "想看它作用在你自己最近的工作上？运行：node bin/ai-collab.js bootstrap --yes（本地只读的「AI 协作基线」报告——不写入也不发送任何东西到任何地方）。\n" +
...
新来的？这三个命令覆盖了你大部分所需（大白话优先）：
  ...
  bootstrap  查看你自己最近的工作，并打印一份只读的「你处在什么位置」报告。init 之后紧接着运行它。
